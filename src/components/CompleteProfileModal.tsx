...
import { Phone, Calendar, User, Sparkles, CheckCircle2, Loader2, ShieldCheck, X } from "lucide-react";
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        {/* Backdrop */}
        <DialogPrimitive.Overlay className="fixed inset-0 z-[9995] bg-black/80 backdrop-blur-md transition-opacity duration-200" />

        {/* Flexbox Centered Container */}
        <div className="fixed inset-0 z-[9996] flex items-center justify-center p-3 sm:p-4 pointer-events-none">
          <DialogPrimitive.Content className="pointer-events-auto max-w-md w-full bg-[#0A0E1A] border border-white/20 text-white rounded-3xl p-5 sm:p-8 shadow-[0_10px_50px_rgba(0,0,0,0.9)] overflow-y-auto max-h-[90vh] backdrop-blur-2xl relative outline-none">
            {/* Glow de fondo */}
            ...
            <div className="flex items-center justify-between relative z-10 mb-2">
              ...
              <DialogPrimitive.Close className="p-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white transition-colors cursor-pointer outline-none">
                <X className="w-4 h-4" />
              </DialogPrimitive.Close>
            </div>

            <DialogPrimitive.Title className="text-2xl font-black font-display text-white tracking-tight relative z-10 text-left">
              Completá tu Perfil
            </DialogPrimitive.Title>
            <DialogPrimitive.Description className="text-xs text-white/60 leading-relaxed relative z-10 text-left mt-1">
            ...
            <div className="mt-4 pt-3 border-t border-white/5 flex items-center gap-2 text-[10px] text-white/40 text-left">

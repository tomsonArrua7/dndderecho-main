...
export default function AsistenteDND() {
  ...
  // Tab states and filters for Study Guide
    ...
    <div className="container py-8 md:py-12 max-w-6xl flex-1 flex flex-col min-h-[calc(100vh-3.5rem)] selection:bg-accent/30">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 items-stretch">
        ...
        <Card className="lg:col-span-8 bg-[#0D1224]/80 backdrop-blur-xl border border-white/5 rounded-2xl flex flex-col h-[600px] overflow-hidden shadow-2xl">
          ...
          {/* Header del Panel con Selector de Pestañas */}
          <div className="px-5 py-4 border-b border-white/5 bg-[#10162D] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-lg bg-accent/20 border border-accent/30 flex items-center justify-center text-accent">
                {activeTab === "chat" ? <Bot className="h-4.5 w-4.5" /> : <BookMarked className="h-4.5 w-4.5" />}
              </div>
              <div>
                <div className="font-bold text-white text-sm">Asistente & Biblioteca DND</div>
                <div className="text-[10px] text-white/40 uppercase tracking-wider font-bold">
                  {selectedMateria ? `${selectedMateria}` : "Selecciona una materia"}
                </div>
              </div>
            </div>

            {/* Pestañas (Tabs) */}
            <div className="flex bg-black/30 border border-white/5 p-1 rounded-xl shrink-0 self-start sm:self-center">
              <button
                type="button"
              ...
              >
                <FolderOpen className="h-3.5 w-3.5" /> Guía de Estudio (PDFs)
              </button>
              <button
                type="button"
              ...
              >
                <Bot className="h-3.5 w-3.5" /> Tutor IA (Preguntas)
              </button>
            </div>
          </div>
          ...
          {activeTab === "links" ? (
            /* =================================================================
               PESTAÑA 1: GUÍA DE ESTUDIO (BIBLIOTECA DE ENLACES PUNTUALES)
               ================================================================= */
            <div className="flex-1 flex flex-col min-h-0 bg-black/10">
              {!selectedMateria ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-4">
                  ...
                  <div className="max-w-sm space-y-1.5">
                    <p className="text-white font-bold text-sm">Biblioteca de la Cátedra</p>
                    <p className="text-white/40 text-xs leading-relaxed">
                      Selecciona tu materia en la barra lateral para explorar sus carpetas oficiales de Drive y ver los programas o resúmenes de descarga directa.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="flex-1 flex flex-col min-h-0 p-5 space-y-5 overflow-y-auto">
                  ...
                  {/* Grid de Carpetas Oficiales */}
                  <div>
                    <h4 className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-3">Carpetas Oficiales de Drive</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[9px] text-white/40 truncate">Ver carpeta de Drive</div>
                        </div>
                      </a>
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-3 bg-white/5 hover:bg-accent/20 border border-white/5 hover:border-accent/30 rounded-xl flex items-center gap-3 transition-all duration-200 group group-hover:text-accent cursor-pointer"
                      >
                  ...
                  <div className="relative shrink-0">
                    ...
                    <input
                      type="text"
                      placeholder="Buscar programa, resumen o archivo específico en esta materia..."
                      value={filterQuery}
                  ...
                  <div className="space-y-4 flex-1">
                    {archivosMateria.length === 0 ? (
                      <div className="p-4 bg-white/5 border border-white/5 rounded-xl text-center text-xs text-white/40">
                        No hay archivos individuales listados para esta materia en la biblioteca de enlaces. Puedes ver todo el contenido en las carpetas de Drive generales arriba.
                      </div>
                    ) : Object.keys(archivosPorCategoria).length === 0 ? (
                      <div className="p-4 bg-white/5 border border-white/5 rounded-xl text-center text-xs text-white/40">
                        Ningún archivo coincide con la búsqueda "{filterQuery}".
                      </div>
                    ) : (
                            ...
                            {lista.map((archivo, aIdx) => (
                              ...
                              >
                                <div className="flex items-center gap-3 min-w-0">
                                  <div className="h-8 w-8 rounded-lg bg-accent/10 border border-accent/20 text-accent flex items-center justify-center shrink-0 text-xs font-bold">
                                    PDF
                                  </div>
                                  <div className="min-w-0">
              ...
              {/* Footer Informativo de la Guía */}
              <div className="px-5 py-3.5 border-t border-white/5 bg-white/5 flex items-center justify-between text-xs text-white/50 shrink-0">
                <span className="flex items-center gap-1.5">
                  <AlertCircle className="h-3.5 w-3.5 text-accent" />
                  Navegación 100% libre de cuotas y límites.
                </span>
                <button
                  type="button"
                  onClick={() => setActiveTab("chat")}
            ...
            /* =================================================================
               PESTAÑA 2: TUTOR INTELIGENTE (IA CHAT CON MEMORIA Y RAG)
               ================================================================= */
            <div className="flex-1 flex flex-col min-h-0 bg-black/10">
